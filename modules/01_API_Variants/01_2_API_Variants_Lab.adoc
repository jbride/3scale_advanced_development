...
$ curl -k "https://`oc get route stores-soap-policy-staging-route -o template --template {{.spec.host}} -n $OCP_USERNAME-3scale-amp`/StoresWS?wsdl&user_key=$STORES_SOAP_API_KEY"
-----
