...
You'll be invoking backend SOAP services via your API gateway.
...
In this section, you leverage the SOAP APIcast gateway _policy_ to track invocations on specific SOAP operations of your backend SOAP service.
...
These `SOAPAction` header fields are now used by your APIcast gateway to update appropriate API _metrics_.
...
The SOAP policy of the 3scale API gateway make use of custom metrics.
Subsequently it is a good candidate to take a quick detour to shed light on these questions.

==== Path based Methods and the _Service Management API_
...
The API gateways does so by invoking the _Service Management API_ of 3scale.
...
The SOAP policy functionality enabled in the API gateway submits an _AuthRep_ request to the _Service Management API_ with an additional increment on the custom metric corresponding to the SOAP operation invoked on the backend SOAP service.
...
The analytics charts provided by 3scale will then display this custom _createStore_ metric having been incremented. 
