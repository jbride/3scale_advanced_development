...
* `3scale-mt-amp0`: Your RHOCP user has _view_ access to the central multi-tenant 3scale API Management system.
...
It is technically feasible that your back-end services, if they are co-located in the same cluster as the 3scale API Management system, can be managed by these default 3scale API Management gateways.
...
. Note that the value of `$THREESCALE_PORTAL_ENDPOINT` is already set for you in each of your gateways:
+
[source,sh]
-----
...
* The API Gateway uses the value of `THREESCALE_PORTAL_ENDPOINT` to invoke the 3scale API Management system and retrieve details of your APIs.
