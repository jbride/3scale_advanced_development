...
echo "export GW_PROJECT=\$API_TENANT_USERNAME=-gw                                               #   Name of OCP namespace where API gateways resides" >> ~/.bashrc
...
[source,texinfo]
-----
$ echo -en "\n\nhttps://`oc get route $API_TENANT_USERNAME=-$API_MANAGER_NS-provider --template {{.spec.host}} -n $API_MANAGER_NS`\n"
-----
...
[source,sh]
-----
$ oc describe deploy prod-apicast -n $API_TENANT_USERNAME=-gw | grep THREESCALE_PORTAL_ENDPOINT
