...
[source,sh]
-----
$ oc rollout resume deploy stage-apicast prod-apicast wc-apicast -n $GW_PROJECT
-----


==== Wildcard API router
Your lab environment comes provisioned with a link:https://github.com/3scale/wildcard-router-service[wildcard-router-service].
...
If you choose to make use of your wildcard router service (and not created additional OCP routes to your gateway for each API), then the following are coouple of considerations:

. *Public Base URLs*
+
The values you set in the _Integration_ page of your _Staging Public Base URL_ and _Production Public Base URL_ must by of the following convention:
+
...
+
The wildcard certificte from LetsEncrypt is valid for the host URLs you specify when using the wildcard API router.
In the case of your wildcard API router routes, they will be secured using a self-signed certificate.
Subsequently, when using the wildcard API router, your HTTP clients will have to ignore errors related to unkown certificate authorities. ie;   using the "-k" with the cURL utility.
