...
* User 3scale Service Discovery to discover to OData service running on OpenShift
...
=== Introduction to Service Discovery of 3scale

In order for any OpenShift service to be discoverable by 3scale AMP running on the same OpenShift cluster, the `default` service account of 3scale AMP namespace needs view access to the cluster. This has already been provisioned for you by the administrator.
