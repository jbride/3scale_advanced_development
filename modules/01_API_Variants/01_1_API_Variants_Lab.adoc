...
** SOAP Policy to automatically associate metrics to SOAP operations
...
. If you are not already there, change to the `$OCP_PROJECT_PREFIX-stores-api` project:
...
+
Create a new edge secure route in the `3scale_AMP` project to `stores-staging-apicast-$OCP_PROJECT_PREFIX.$OCP_WILDCARD_DOMAIN`, mapped to the `apicast-staging` service.

* *Production Public Base URL*: 
+
Create a new route in the `3scale_AMP` project to `stores-production-apicast-$OCP_PROJECT_PREFIX.$OCP_WILDCARD_DOMAIN`, mapped to the `apicast-production` service.
