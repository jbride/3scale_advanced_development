...
.. As per the link:https://www.w3.org/TR/2000/NOTE-SOAP-20000508/#_Toc478383528[SOAP 1.1 specification], an HTTP client *MUST* use this header filed when issuing a SOAP HTTP Request. 
...
Similar to the previous section of this lab, you will continue to make use of the HTTP header called _SOAPAction_.

In this section, however, you will configure custom policies that indicate to the APIcast gateway how to map the values of the _SOAPAction_ header to custom API management metrics.
...
By doing so, you'll then be able to visualize your invocations to each SOAP operation in the build-in analytics capabilities of 3scale.
...
. Alternatively, it can be easier to execute the SOAP requests using SOAP-UI.
+
Similar to the _curl_ utility, SOAP-UI does allow for setting of the custom _SOAPAction_ header in the http request. 
+
image::images/3scale_amp_stores_soap_soapui.png[]
+
If you decide to use SOAP-UI, please refer to the appendix of this lab for tips regarding the import of the _Store_ API WSDL.

. Whether using either the _curl_ utility or SOAP-UI, make several invocations to each of the four SOAP operations of the _Stores_API.
