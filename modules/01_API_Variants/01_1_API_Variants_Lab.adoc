...
We'll do this by adding a custom header called _SOAPAction_ to each HTTP request.

The APIcast gateway of 3scale implements its SOAP operation tracking feature by keying in on SOAP requests that include this _SOAPAction_ custom header.
