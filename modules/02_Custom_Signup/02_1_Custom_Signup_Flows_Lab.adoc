...
.. The API Consumer could send requests using credentials associated with this automatically generated _application_ to the API Gateway 

The remainder of the exercises in this lab assume API services that do not define a _default_ application plan.
Navigate to the _Application Plans_ section of each of your APIs to verify this is the case.

Because your services do not specify a default Application Plan, an _application_ that an API Consumer can get started with will not be created.
Instead, the signup flow via the Developer Portal involves requiring the API Consumer to select specific applications plans to signup for.
