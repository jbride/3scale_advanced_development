...
* Should your service(s) specify a  _default application plan_?
...
For example, the out of the box api that comes pre-provisioned with the creation of a new 3scale tenant specifies a _Default Plan_ called:  `Basic`

image::images/default_app_plan.png[]

Subsequently, upon creation of a new _account_, a new _application_ derived from the _Basic_ application and the account is created.
...
. API Consumer navigate to the Developer Portal of your API
. API Consumer creates an account
.. An _application_ derived from that API Consumer's account and the _default application plan_ of each API service is created.

The remainder of the exercises in this lab assume API services that do not define a default application plan.
Subsequently, the signup flow to your Developer Portal involves requiring the API Consumer to select specific applications plans from the APIs offered by your tenant.
...
+
image::images/3scale_amp_custom_signup_single_plan_3.png[width="90%"]
