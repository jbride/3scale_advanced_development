...
Instead of relying on application plans for *internal* and *public* user signup, we will restrict the service itself to be either *external* or *private*.
In this manner, the service will only be seen by the users who belong to a particular group.
...
. As the API Consumer, click on the *Applications* menu item.
+
